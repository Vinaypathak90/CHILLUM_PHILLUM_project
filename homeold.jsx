...
const SectionHeader = ({ label, titleMain, titleHighlight, titleEnd, align = 'left' }) => (
  <div style={{ textAlign: align, maxWidth: align === 'center' ? '100%' : '680px', margin: align === 'center' ? '0 auto' : '0' }}>
...
const Home = () => {
  ...
  // 2. Animations & DOM Manipulation (Runs after data loads)
  useEffect(() => {
    if (loading) return;

    // A. Active Nav Link & Shrink Header on Scroll
    const handleScroll = () => {
    ...
    // C. Number Counter Animation (For Stats)
    ...
    const counterObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          const suffix = raw.replace(/[0-9]/g, ''); // Extracts '+' or '%'
          let count = 0;
  ...
  // Prevent rendering until initial fetch is complete
  ...
  // 🚀 DYNAMIC STATS LOGIC (With default fallback just in case database is empty)
  const displayStats = content.about?.stats?.length > 0 ? content.about.stats : [
    ...
    <>
      ...
      <section id="home" onMouseMove={handleSpotlight}>
        <div className="hero-strip" aria-hidden="true">
          <div className="hero-strip-track">
            {/* Duplicating images to create seamless infinite scroll loop */}
            {[...displayHeroImages, ...displayHeroImages, ...displayHeroImages].map((img, i) => (
              <div className="hero-strip-item" key={i}><img src={img} alt="Hero Background"/></div>
            ))}
          </div>
        </div>
      ...
      <section id="about">
        ...
        <div className="about-images">
          {content.about?.images?.map((img, i) => img && (
            <div className="about-img" key={i}><img src={img} alt="Studio"/></div>
          ))}
        </div>
      </section>
      ...
      <section id="studio">
        ...
        <div className="studio-grid">
          {content.studio?.cards?.map((card, i) => card.label && (
            <div className="studio-card" key={i} onMouseMove={(e) => applyTilt(e, 6)} onMouseLeave={removeTilt}>
      ...
      <section id="projects">
        <div className="projects-header">
          ...
          <a href="#" className="btn-gold">View All</a>
        </div>
        <div className="projects-grid">
          {data.projects.length > 0 ? data.projects.map((project) => (
            <div className="project-card" key={project._id} onMouseMove={(e) => applyTilt(e, 8)} onMouseLeave={removeTilt}>
          ...
          )) : <p style={{color: '#888', padding: '0 3rem'}}>Projects will appear here.</p>}
        </div>
      </section>
      ...
      <section id="news">
        <div className="news-header">
          ...
          <a href="#" className="btn-gold">All News</a>
        </div>
        <div className="news-grid">
          {data.campaigns.length > 0 ? data.campaigns.map((camp) => (
            <div className="news-card" key={camp._id}>
              ...
              <a href={camp.readMoreLink} className="news-read">Read More →</a>
            </div>
          )) : <p style={{color: '#888', padding: '0 3rem'}}>News and campaigns will appear here.</p>}
        </div>
      </section>
      ...
      <section id="contact">
        <div className="contact-info">
          ...
          <div className="divider"></div>
          <p>We're always looking for exciting projects and bold collaborators. Whether you're a filmmaker, brand, or storyteller — reach out and let's make something together.</p>
        ...
        <form className="contact-form" onSubmit={handleContactSubmit}>
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              <label>Subject</label>
              <input type="text" placeholder="Project Inquiry" required value={formData.subject} onChange={(e) => setFormData({...formData, subject: e.target.value})}/>
            </div>
          </div>
          <div className="form-group">
            <label>Message</label>
            <textarea placeholder="Tell us about your project..." required value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})}></textarea>
          </div>
          <button className="form-submit" type="submit" style={{ backgroundColor: formStatus === 'Sent!' ? '#2d7a4f' : '', color: formStatus === 'Sent!' ? '#fff' : '' }}>
            {formStatus}
          </button>
        </form>
      </section>
      ...
      <footer>
        ...
        <p className="footer-copy">© 2026 Chillum Phillum. All rights reserved.</p>
